...
export async function POST(request: NextRequest) {
  try {
    ...
    const aiClient = env.useOllama
      ? new OllamaVisionClient(env.ollamaBaseUrl, env.ollamaVisionModel)
      : new NvidiaVisionClient(env.nvidiaApiKey);
    const useCase = new AnalyzeJournalImage(aiClient);
    const analysis = await useCase.execute(imageBase64, file.type);

    return NextResponse.json(analysis);
  } catch (error) {
    logger.error("Journal analysis failed", error);
    const message = error instanceof Error ? error.message : "상담일지 분석에 실패했습니다.";
